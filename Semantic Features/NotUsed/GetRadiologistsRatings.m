...
RATINGS.subtlety = cell2mat(data(matchingRows, 9));
RATINGS.calcification = cell2mat(data(matchingRows, 10)); %not a 'ranking' number
RATINGS.sphericity = cell2mat(data(matchingRows, 11));
RATINGS.margin = cell2mat(data(matchingRows, 12));
RATINGS.lobulation = cell2mat(data(matchingRows, 13));
RATINGS.spiculation = cell2mat(data(matchingRows, 14));
RATINGS.texture = cell2mat(data(matchingRows, 15));
RATINGS.malignancy = cell2mat(data(matchingRows, 16));
...
RATINGS.sourceDirectory = char(data(matchingRows, 8)); %only need one of the identical entries
...
borderCells = data(matchingRows, 20);
RATINGS.borders = ParseBorderCells(borderCells); 
RATINGS.area = cell2mat(data(matchingRows, 54)); 
